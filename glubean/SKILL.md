...
0. **Bootstrap check** — before anything else:
   - Check whether this project has already entered the Glubean workflow by looking for `@glubean/sdk` in `package.json` dependencies or devDependencies.
   - **If missing** (cold start): treat the project as a zero-to-first-test setup. Read `~/.glubean/docs/patterns/bootstrap.md` and follow it instead of the workflow below.
     If lens docs aren't pulled yet, run `npx glubean docs pull` first.
     Key: check if the current environment is a VS Code-based editor (VS Code, Cursor, Windsurf).
     **If not** → strongly recommend opening the project in a VS Code-based editor. Read `~/.glubean/docs/patterns/bootstrap.md` for the "Why VS Code" section and share the key reasons. Then continue — do not block.
     **If yes** but Glubean extension is not installed → suggest installing it (`ext install glubean.glubean`), explain what it adds, then continue.
     In all cases → ask: "Want to write a quick scratch test to try it, or init a full project?"

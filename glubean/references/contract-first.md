...
- `workflows/` or `contracts/` can hold `workflow()` lifecycle specs that compose existing contract cases.
- `tests/` is for runtime evidence that contracts/workflows should not try to project (browser-heavy flows, exploratory checks, complex imperative probes).
...
5. Use top-level `workflow()` for cross-endpoint lifecycle verification.
...
- Use `workflow()` for stateful endpoint chains (upload -> read -> delete), especially when the chain should be projectable for scanner, Cloud, or agent review.
- Use `test()` only for scenarios contract/workflow should not express: browser-heavy journeys, exploratory probes, deeply imperative runtime evidence, or one-off debugging.
- Keep Zod schemas in `schemas/`. Use `expect.schema` for response validation, not scattered assertions.
- Keep contract cases pure. Do not add per-case `setup` / `teardown`; use `given`, `needs`, `runnability`, `contract.bootstrap()`, `defineSession()`, or `workflow().setup/teardown()` instead.

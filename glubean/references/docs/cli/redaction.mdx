...
Add a `redaction` block under `defaults` in `glubean.yaml`. It's inherited by
every profile:
...
> These rules apply when you run a **profile** (`glubean run --profile <name>`
> or `glubean ci run`). An ad-hoc run with an explicit target
> (`glubean run tests/`) doesn't resolve a profile and uses the built-in
> baseline only — run through a profile to apply your custom rules before an
> upload.

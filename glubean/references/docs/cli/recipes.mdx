...
`glubean ci run` runs the `ci` profile from `glubean.yaml` — fail-fast and
the junit/result-json reporters come from `profiles.ci`. See [Connect CI](/getting-started/ci).

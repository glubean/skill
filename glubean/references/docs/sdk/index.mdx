...
1. [Test Shape](/sdk/test-api) — `test()`, builder API, branch/switch/poll, `test.each`, `test.pick`
2. [Configuration](/sdk/configuration) — `configure()` for shared setup
3. [Contract HTTP](/sdk/contract-http) — `contract.http.with()` endpoint promises
4. [Workflow](/sdk/contract-flow) — `workflow()` lifecycle promises
5. [HTTP Requests](/sdk/http-client) — `ctx.http` client
6. [Assertions](/sdk/assertions) — `expect`, `assert`, `validate`, `warn`
7. [Data-Driven](/sdk/data-driven) — `test.each`, `test.pick`, data loaders
8. [Variables & Secrets](/sdk/env-and-secrets) — `ctx.vars`, `ctx.secrets`

## When the suite grows

- Use the builder API for imperative multi-step tests
- Use `workflow()` when the lifecycle is a product promise worth projecting

...
Always include a diagnostic message. The message should name the invariant being checked, and custom boolean predicates should include `actual` / `expected` details when runtime values are compared.

```typescript
ctx.assert(res.ok, "create user request should return a 2xx response");

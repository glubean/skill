...
Deprecated cases are skipped at runtime (just like deferred), but appear in projection as `🚫` and translate to OpenAPI `deprecated: true` + `x-deprecated-reason`.
...
Merge precedence: `instance defaults < contract < case`. The final merged view appears in the OpenAPI operation as `x-*` fields.

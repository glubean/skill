...
| **How you run** | Pick one case: `glubean run --profile explore --pick by-name` | Run all: `glubean run` |
...
| **CLI default** | Needs `--profile explore` or explicit path | Runs by default (the `local` profile) |

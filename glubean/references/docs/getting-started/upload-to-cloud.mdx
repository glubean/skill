...
For project ID, a profile's `upload.projectAlias` in `glubean.yaml` is also honored when you run that profile — and it takes precedence over `GLUBEAN_PROJECT_ID`. See [Tokens](/cloud/tokens#where-to-store-it) for the exact order.
